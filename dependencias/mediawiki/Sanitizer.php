...
class Sanitizer {
   ...
   */
  const CHAR_REFS_REGEX = '/&([A-Za-z0-9\x80-\xff]+);
		 |&\#([0-9]+);
		 |&\#[xX]([0-9A-Fa-f]+);
		 |(&)/x';
  ...
  static function mergeAttributes($a, $b) {
    $out = array_merge($a, $b);
    if (isset($a['class']) && isset($b['class']) && is_string($a['class']) && is_string($b['class']) && $a['class'] !== $b['class']) {
      $classes = preg_split('/\s+/', "{$a['class']} {$b['class']}", -1, PREG_SPLIT_NO_EMPTY);
  ...
  static function safeEncodeAttribute($text) {
    ...
    $encValue = preg_replace_callback(
            '/((?i)' . wfUrlProtocols() . ')/', array('Sanitizer', 'armorLinksCallback'), $encValue);
    return $encValue;
  }
  ...
  static function escapeClass($class) {
    ...
    return rtrim(preg_replace(
                    array('/(^[0-9\\-])|[\\x00-\\x20!"#$%&\'()*+,.\\/:;<=>?@[\\]^`{|}~]|\\xC2\\xA0/', '/_+/'), '_', $class), '_');
  }
  ...
  public static function decodeTagAttributes($text) {
    ...
    $pairs = array();
    if (!preg_match_all(
                    self::getAttribsRegex(), $text, $pairs, PREG_SET_ORDER)) {
      return $attribs;
    }
  ...
  private static function normalizeAttributeValue($text) {
    return str_replace('"', '&quot;', self::normalizeWhitespace(
  ...
  static function normalizeCharReferences($text) {
    return preg_replace_callback(
            self::CHAR_REFS_REGEX, array('Sanitizer', 'normalizeCharReferencesCallback'), $text);
  }
  ...
  public static function decodeCharReferences($text) {
    return preg_replace_callback(
            self::CHAR_REFS_REGEX, array('Sanitizer', 'decodeCharReferencesCallback'), $text);
  }
  ...
  public static function decodeCharReferencesAndNormalize($text) {
    ...
    $text = preg_replace_callback(
            self::CHAR_REFS_REGEX, array('Sanitizer', 'decodeCharReferencesCallback'), $text, /* limit */ -1, $count);
  ...
  static function setupAttributeWhitelist() {
    ...
    $whitelist = array(
        ...
        'dt' => $common,
        # 11.2.1
        'table' => array_merge($common, array('summary', 'width', 'border', 'frame',
